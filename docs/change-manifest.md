...
| **K-16** | **Pin `accountType: 'eoa'` at wallet creation, and constrain the schema to EOA** | `[FIX]` | ⚠️ **Justification corrected 2026-08-05 — see `docs/decision-wallet-model.md`.** The original rationale here (*"Gateway rejects non-EOA signatures… EIP-1271 signatures can't be accepted"*, and *"Nanopayments is EOA-only"*) was **uncited and is contradicted by Circle's Agent Nanopayments docs**, which document Gateway Nanopayments for agent wallets — and `circle wallet create` makes **SCA** wallets. Do not rely on that reasoning. Pinning EOA remains correct because developer-controlled wallets are EOA by construction and the DB should reject a wrong value early; migration `0025` does this. |
