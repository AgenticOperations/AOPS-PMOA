...
6. **Solvency measures the treasury WALLET, not Circle Gateway — they are different pools.** Found while wiring Task 9. `assertDelegationWithinCeilings` reads `nativeBalanceMicros(treasuryAddress)`, i.e. the treasury's own on-chain balance. But the existing allocation path funds agents out of **Circle Gateway**, and `demo/reset.mjs` deposits nearly the whole treasury balance into Gateway (`TREASURY_ARC_DEPOSIT_USDC = '8.00'` against a `TREASURY_ARC_MIN_MICROS = 8_000_000` wallet floor) — and, as its own comment says, *"every USDC deposited to Gateway leaves the wallet for good."*
   ...
   So an org funded the Gateway way has a near-empty treasury wallet and every treasury delegation is refused with `treasury_insufficient_for_ceiling`, even though the org demonstrably has funds. The two funding models do not compose today. This is why `demo/reset.mjs` was deliberately **not** updated to seed treasury delegations: doing so would have made the demo fail on a live run.

   **RESOLVED — the ceiling counts the treasury wallet balance only.** Gateway funds cannot back a Permit2 pull, so counting them would let the console show an org as solvent right up until every delegation was refused. The engine already behaved this way; the follow-up work made the console honest about it rather than changing the rule:

   - `GET /payments/delegations/ceiling` now returns `treasury_balance_usdc` — the wallet balance, Gateway excluded. It degrades to `null` (not a 500) when the chain RPC is down, since the ceiling and outstanding headroom beside it are pure SQL and stay correct.
   - The delegations page shows that balance next to the copyable treasury deposit address, and warns when the balance — not the ceiling — is the binding constraint, so an operator does not raise a ceiling that will have no effect.
   - Payments → Sources: the button labelled **"Deposit"** was moving USDC *out* of the treasury into Gateway. Renamed **"Move to Gateway"**, with the one-way-door consequence stated in the drawer. The old label made a mis-click that silently breaks every treasury delegation one word away from the correct action.
