...
function fakeProvider(overrides: Partial<CircleTreasuryProvider> = {}): CircleTreasuryProvider {
  return {
    ...
    executePermit2Transaction: vi.fn(() => Promise.resolve({ txHash: '0x1a1a5cf5e6ff44be16f56fbd3e78f5e120de3d5d4c2c26e7c8d19e5b8e14c6f2' })),
...
describe('Writer demo agent, end to end', () => {
  ...
  beforeAll(async () => {
    ...
    walletAddress = '0xdada00000000000000000000000000000000da7b';
    ...
    const { createWriterAgent } = writerModule;
    agent = createWriterAgent({ walletAddress, rpcUrl: 'https://rpc.testnet.arc.network' });
    const address = await agent.listen({ host: '127.0.0.1', port: 0 });
    agentUrl = `${address}/report?topic=quarterly-metrics`;
  ...
  it('pays the agent via a real payIntraFleet drawdown and receives the written report', async () => {
    const orgId = 'org_demo_writer';
    const teamId = 'team_demo_writer';
    const payerAgentId = 'agt_demo_writer_payer';
    const payeeAgentId = 'agt_demo_writer';
    const walletSetId = 'ws_demo_writer';

    await store.pool.query("INSERT INTO orgs (id, display_name) VALUES ($1, 'Demo Writer Org')", [orgId]);
    ...
    await store.pool.query('INSERT INTO agents (id, org_id, team_id, name) VALUES ($1, $2, $3, $4)', [payeeAgentId, orgId, teamId, 'Writer']);
    ...
    await recordProvisionedWallet(store.pool, {
      ...
      circleWalletId: 'w_demo_writer_payer', address: '0xdada00000000000000000000000000000000fea2',
      refId: 'ref_demo_writer_payer', walletSetId, circleBlockchain: 'ARC-TESTNET',
    });
    await recordProvisionedWallet(store.pool, {
      orgId, agentId: payeeAgentId, mode: 'test', chain: 'arc',
      circleWalletId: 'w_demo_writer', address: walletAddress,
      refId: 'ref_demo_writer', walletSetId, circleBlockchain: 'ARC-TESTNET',
    });
