...
import type { OperationalDecisionRecord } from '@/lib/operations-types';
...
/** @deprecated Prefer OrgActivityTab — kept for TreasuryActivity re-exports. */
export type TreasuryActivityTab = OrgActivityTab;

type OrgActivityWorkbenchProps = {
  readonly activeTab: OrgActivityTab;
...
function outcomeTone(outcome: string): string {
  ...
  if (['accepted', 'active', 'approved', 'allow', 'complete', 'completed', 'consumed', 'delivered', 'funded', 'observe', 'settled', 'success'].includes(normalized)) return 'active';
  if (['blocked', 'cancelled', 'denied', 'deny', 'error', 'expired', 'failed', 'rate_limited', 'rate limited', 'rejected', 'revoked'].includes(normalized)) return 'failed';
  if (['approval_required', 'approval required', 'open', 'pending', 'queued', 'reserved', 'submitted', 'warning'].includes(normalized)) return 'pending';
  return 'inactive';
}
...
function isPolicyLifecycleEvent(event: AuditEventRecord): boolean {
...
function rowsForTab(props: OrgActivityWorkbenchProps): EvidenceRow[] {
  if (props.activeTab === 'payments') {
    return props.paymentEvents.map((event) => {
  ...
  if (props.activeTab === 'routing') {
    return props.routeObservations.map((observation) => ({
      amount: formatMoney(observation.amount_usdc),
      chain: observation.supported_rail === null ? null : observation.supported_rail.replace(/^gateway_|^exact_/, '') as PaymentChain,
      id: observation.id,
      outcome: titleCase(observation.outcome),
      primary: observation.resource_category ?? observation.requested_asset ?? 'x402 request',
      secondary: observation.agent_id,
      target: formatOptionalRail(observation.supported_rail),
      time: observation.observed_at,
      details: [
        { label: 'Reason', value: observation.reason_code },
        { label: 'Requested network', value: observation.requested_network ?? 'Not recorded' },
        { label: 'Requested rail', value: observation.requested_rail ?? 'Not recorded' },
        { label: 'Resource URL', value: observation.resource_url ?? 'Not recorded' },
      ],
    }));
  }
  if (props.activeTab === 'reservations') {
    return props.reservations.map((reservation) => ({
      amount: formatMoney(reservation.amount_usdc),
      chain: reservation.rail.replace(/^gateway_|^exact_/, '') as PaymentChain,
      id: reservation.id,
      outcome: titleCase(reservation.status),
      primary: reservation.reason_code,
      secondary: reservation.agent_id,
      target: formatRail(reservation.rail),
      time: reservation.updated_at,
      details: [
        { label: 'Quote hash', value: reservation.quote_hash },
        { label: 'Source', value: reservation.source_id },
        { label: 'Expires', value: formatUtcDateTime(reservation.expires_at) },
        { label: 'Connection', value: reservation.connection_id ?? 'None' },
      ],
    }));
  }
  if (props.activeTab === 'jobs') {
    return props.providerJobs.map((job) => ({
      amount: formatMoney(job.amount_usdc),
      chain: job.chain,
      id: job.id,
      outcome: titleCase(job.status),
      primary: titleCase(job.job_type.replaceAll('.', '_')),
      secondary: job.chain === null ? 'Workspace' : CHAIN_LABELS[job.chain],
      target: job.provider_ref ?? 'Provider job',
      time: job.updated_at,
      details: [
        { label: 'Provider reference', value: job.provider_ref ?? 'Not assigned' },
        { label: 'Error', value: job.error_code ?? 'None' },
        { label: 'Created', value: formatUtcDateTime(job.created_at) },
        { label: 'Mode', value: titleCase(job.mode) },
      ],
    }));
  }
  ...
  if (props.activeTab === 'decisions') {
    return (props.decisions ?? []).map((decision) => ({
      amount: formatActionLabel(decision.action),
      chain: null,
      id: decision.id,
      outcome: formatDecisionLabel(decision.decision),
      primary: decision.tool_name ?? decision.resource_label ?? decision.action,
      secondary: decision.agent_id,
      target: decision.reasonCode,
      time: decision.created_at,
      details: [
        { label: 'Explanation', value: decision.explanation },
        { label: 'Policy decision', value: decision.policyDecisionId ?? 'None' },
        { label: 'Approval', value: decision.approvalId ?? 'None' },
        { label: 'Connection', value: decision.connection_id ?? 'None' },
        { label: 'Resource domain', value: decision.resource_domain ?? 'Not recorded' },
        { label: 'Resource category', value: decision.resource_category ?? 'Not recorded' },
        { label: 'Tool risk', value: decision.tool_risk_level ?? 'Not recorded' },
      ],
    }));
  }
  if (props.activeTab === 'approvals') {
    return (props.approvals ?? []).map((approval) => ({
      amount: approval.action_id,
      chain: null,
      id: approval.id,
      outcome: titleCase(approval.status),
      primary: approval.target_type,
      secondary: approval.agent_id,
      target: approval.target_id ?? (approval.note || 'Approval'),
      time: approval.updated_at,
      details: [
        { label: 'Decision id', value: approval.decision_id },
        { label: 'Connection', value: approval.connection_id },
        { label: 'Requested by', value: approval.requested_by },
        { label: 'Approved by', value: approval.approved_by ?? '—' },
        { label: 'Denied by', value: approval.denied_by ?? '—' },
        { label: 'Expires', value: formatUtcDateTime(approval.expires_at) },
        { label: 'Note', value: approval.note || 'None' },
      ],
    }));
  }
  if (props.activeTab === 'policy') {
    return props.auditEvents.filter(isPolicyLifecycleEvent).map((event) => ({
      amount: event.eventCategory,
      chain: null,
      id: event.id,
      outcome: titleCase(event.outcome),
      primary: titleCase(event.eventType.replaceAll('.', '_')),
      secondary: `${event.actorType}:${event.actorId ?? 'system'}`,
      target: event.resourceType === null ? event.eventDomain : `${event.resourceType}:${event.resourceId ?? 'unknown'}`,
      time: event.occurredAt,
      details: [
        { label: 'Action', value: event.action },
        { label: 'Policy ref', value: event.policyRef ?? 'Not recorded' },
        { label: 'Reason', value: event.reasonCode ?? 'Not recorded' },
        { label: 'Event hash', value: event.eventHash },
        { label: 'Source', value: event.sourceSystem ?? 'Not recorded' },
        { label: 'Severity', value: titleCase(event.severity) },
      ],
    }));
  }
  return props.auditEvents.map((event) => ({
    amount: event.eventCategory,
    chain: null,
    id: event.id,
    outcome: titleCase(event.outcome),
    ...
    details: [
      { label: 'Event hash', value: event.eventHash },
      { label: 'Previous hash', value: event.previousHash ?? 'Genesis' },
      { label: 'Canonical body hash', value: event.canonicalBodyHash },
      { label: 'Source', value: event.sourceSystem ?? 'Not recorded' },
      { label: 'Severity', value: titleCase(event.severity) },
    ],
  }));
...
const tabs: readonly { readonly key: OrgActivityTab; readonly label: string }[] = [
...
function tabHref(orgSlug: string, tab: OrgActivityTab): string {
  return tab === 'payments' ? `/app/${orgSlug}/activity` : `/app/${orgSlug}/activity?tab=${tab}`;
}

function emptyCopy(tab: OrgActivityTab): { readonly title: string; readonly body: string } {
  if (tab === 'escrow') {
    return {
      title: 'No escrow yet',
      body: 'Escrow jobs appear here after marketplace hire.',
    };
  }
  if (tab === 'decisions') {
    return {
      title: 'No runtime decisions yet',
      body: 'Allow, deny, and rate-limit decisions appear when agents hit Operations controls.',
    };
  }
  if (tab === 'approvals') {
    return {
      title: 'No approvals yet',
      body: 'Pending and resolved approval requests appear here. Resolve them in Approvals.',
    };
  }
  if (tab === 'policy') {
    return {
      title: 'No policy changes yet',
      body: 'Activate, revise, or archive policies in Controls to build this change log.',
    };
  }
  return {
    title: `No ${tab} yet`,
    body: 'Records appear when agents spend, route, or the org records audit events.',
  };
}

export function OrgActivityWorkbench(props: OrgActivityWorkbenchProps) {
  ...
  const showChainFilter = props.activeTab !== 'decisions'
  ...
  const filteredRows = rows.filter((row) => {
    ...
    if (showChainFilter && chainFilter !== 'all' && row.chain !== chainFilter) return false;
  ...
  const empty = emptyCopy(props.activeTab);

  return (
    <div className="treasury-workbench treasury-activity-workbench org-activity-workbench">
      <header className="treasury-page-header">
        <div>
          <p className="treasury-eyebrow">Runtime</p>
          <h1>Activity</h1>
          <p>Payments, escrow, audit, decisions, approvals, and policy changes — one place with filters.</p>
        </div>
      </header>

      <nav aria-label="Activity type" className="treasury-evidence-tabs">
        {tabs.map((tab) => (
          <Link
            aria-current={props.activeTab === tab.key ? 'page' : undefined}
            className={props.activeTab === tab.key ? 'is-active' : undefined}
            href={tabHref(props.orgSlug, tab.key)}
            key={tab.key}
          >
            {tab.label}
          </Link>
      ...
      <div className="treasury-table-toolbar treasury-activity-toolbar">
        <label className="treasury-search-field">
        ...
        {showChainFilter ? (
          <label>
            ...
            <select
              onChange={(event) => {
                setChainFilter(event.target.value as 'all' | PaymentChain);
                setPage(1);
              }}
              value={chainFilter}
            >
      ...
      <section aria-labelledby="org-activity-title" className="treasury-table-section">
        <div className="treasury-section-heading">
          <div>
            <h2 id="org-activity-title">{tabs.find((tab) => tab.key === props.activeTab)?.label}</h2>
          </div>
        ...
        {rows.length === 0 ? (
          ...
          <>
            <TableShell className="treasury-table-shell">
              <Table aria-label={`${props.activeTab} activity`} className="treasury-table">
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>
                      {props.activeTab === 'escrow'
                        ? 'Paying agent'
                        : props.activeTab === 'approvals' || props.activeTab === 'decisions'
                          ? 'Agent / target'
                          : 'Agent / operation'}
                    </TableHead>
                    <TableHead>
                      {props.activeTab === 'escrow'
                        ? 'Provider'
                        : props.activeTab === 'decisions' || props.activeTab === 'approvals' || props.activeTab === 'policy'
                          ? 'Detail'
                          : 'Rail / target'}
                    </TableHead>
                    <TableHead>
      ...
      <Sheet
        labelledBy="org-activity-drawer-title"
      ...
      >
        <SheetHeader>
          <div>
            <SheetTitle id="org-activity-drawer-title">Record</SheetTitle>
            <SheetDescription>{selected?.primary ?? 'Activity detail'}</SheetDescription>
          </div>
          ...
          {selected === undefined ? null : (
            <>
              <div className="treasury-drawer-status">
                <StatusBadge label={selected.outcome} status={outcomeTone(selected.outcome)} />
