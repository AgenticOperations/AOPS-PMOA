...
export function TreasuryActivity(props: TreasuryActivityProps) {
  ...
  return (
    <div className="treasury-workbench treasury-activity-workbench">
      ...
      <TreasuryPageHeader description="Independent evidence views keep payment, routing, reservation, provider, and audit records addressable without mixing their contracts." eyebrow="Treasury / activity & evidence" title="Every payment has a trail." />

      <nav aria-label="Treasury evidence type" className="treasury-evidence-tabs">{tabs.map((tab) => <Link aria-current={props.activeTab === tab.key ? 'page' : undefined} className={props.activeTab === tab.key ? 'is-active' : undefined} href={`/app/${props.orgSlug}/payments/activity${tab.key === 'payments' ? '' : `?tab=${tab.key}`}`} key={tab.key}>{tab.label}</Link>)}</nav>

      <div className="treasury-table-toolbar treasury-activity-toolbar">
        <label className="treasury-search-field"><IconSearch aria-hidden="true" size={15} stroke={1.8} /><span className="sr-only">Search evidence</span><input onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Search reference, agent, recipient, or resource…" value={query} /></label>
      ...
      <Sheet labelledBy="treasury-evidence-drawer-title" onOpenChange={(open) => !open && setSelectedId(null)} open={selected !== undefined} panelClassName="treasury-drawer">
        <SheetHeader><div><SheetTitle id="treasury-evidence-drawer-title">Evidence record</SheetTitle><SheetDescription>{selected?.primary ?? 'Treasury evidence and provider references.'}</SheetDescription></div><SheetCloseButton onClick={() => setSelectedId(null)} /></SheetHeader>
        <SheetBody className="treasury-evidence-drawer">{selected === undefined ? null : <><div className="treasury-drawer-status"><StatusBadge label={selected.outcome} status={outcomeTone(selected.outcome)} /><code>{selected.id}</code></div><dl className="treasury-evidence-grid"><div><dt>Agent / operation</dt><dd>{selected.secondary}</dd></div><div><dt>Rail / target</dt><dd>{selected.target}</dd></div><div><dt>Amount / change</dt><dd>{selected.amount}</dd></div><div><dt>Recorded</dt><dd>{formatUtcDateTime(selected.time)}</dd></div>{selected.details.map((detail) => <div key={detail.label}><dt>{detail.label}</dt><dd>{detail.value}</dd></div>)}</dl></>}
        </SheetBody>
      </Sheet>
    </div>
  );
